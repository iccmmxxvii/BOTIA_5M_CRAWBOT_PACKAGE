...
class DataFeed:
    ...
    def __init__(self, config: Dict):
        ...
        # Market interval (default 15m=900s). For BTC 5m use interval_sec=300 and interval_tag='5m' in config.
        market_cfg = self.config.get('market', {}) if isinstance(self.config, dict) else {}
        self.interval_sec = int(market_cfg.get('interval_sec', 900))
        self.interval_tag = str(market_cfg.get('interval_tag', '15m'))
        ...
        self.markets = {
            'btc': {
                ...
                'tokens': {},
                'seconds_till_end': self.interval_sec,
                'market_end_time': int(time.time()) + self.interval_sec,
    ...
    def _current_slug(self, coin: str) -> str:
        ...
        current_slot = int(time.time()) // self.interval_sec * self.interval_sec
        return f"{coin}-updown-{self.interval_tag}-{current_slot}"
    ...
    def _fetch_tokens(self, coin: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not events:
                ...
                next_market = ((current_time // self.interval_sec) + 1) * self.interval_sec
    ...
    def _polymarket_worker(self, coin: str):
        ...
        while not self.stop_event.is_set():
            ...
            current_time = int(time.time())
            market_end = ((current_time // self.interval_sec) * self.interval_sec) + self.interval_sec
