...
def run_manual_redeem():
    ...
    try:
        ...
        import sys
        # Optional legacy path (ignored if missing)
        ...
        print(f"[REDEEM] Using wallet from: {PROJECT_ROOT / '.env'}")
        print()
...
def main():
    ...
    def handle_chart_command():
        ...
        try:
            ...
            import uuid
            chart_path = str(LOG_DIR / f"pnl_chart_on_demand_{uuid.uuid4().hex[:8]}.png")
            ...
            print(f"[TELEGRAM CMD] Log dir: {LOG_DIR}")
            ...
            result = generate_pnl_chart(str(LOG_DIR), COINS, chart_path)
            ...
            with market_lock:
                ...
                for coin in COINS:
                    trades_file = (LOG_DIR / f"late_v3_{coin}/trades.jsonl")
                        ...
                        try:
                            ...
                            if result:
                                ...
                                if total_completed_markets - last_chart_at >= CHART_INTERVAL:
                                    ...
                                    chart_path = str(LOG_DIR / f"pnl_chart_{total_completed_markets}.png")
                                    from pnl_chart_generator import generate_pnl_chart
                                    if generate_pnl_chart(str(LOG_DIR), COINS, chart_path):
